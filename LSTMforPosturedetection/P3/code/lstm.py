...
def build_and_train_model(activation_function):
    ...
    model = Sequential()
    model.add(LSTM(100, input_shape=(X_train.shape[1], X_train.shape[2]), activation=activation_function))
    ...
    # 3. Train the model
    history = model.fit(X_train, y_train, epochs=99, batch_size=32, validation_data=(X_val, y_val), shuffle=True, callbacks=[early_stop])
